# Ralph Loop - 程式碼架構總結

## 核心設計原則（基於 ralph-claude-code）

### 1. 迴圈控制的三個層次

```
┌─────────────────────────────────┐
│   應用程式層 (app)              │
│   - 整合所有模組                 │
│   - 實作業務邏輯                 │
└────────────┬────────────────────┘
             │
┌────────────▼────────────────────┐
│   迴圈控制層 (ghcopilot)         │
│   ├─ CLI 執行器                 │
│   ├─ 輸出解析器                 │
│   ├─ 回應分析器 (ralph 方式)    │
│   ├─ 熔斷器                     │
│   └─ 退出偵測                   │
└────────────┬────────────────────┘
             │
┌────────────▼────────────────────┐
│   CLI 層 (github-copilot-cli)   │
│   - 執行 what-the-shell         │
│   - 執行其他指令                 │
└─────────────────────────────────┘
```

### 2. 雙重條件退出驗證 🔑

```go
func (ra *ResponseAnalyzer) IsCompleted() bool {
    // 條件 1: 有足夠的完成指標（分數 >= 20）
    if len(ra.completionIndicators) < 2 {
        return false
    }
    
    // 條件 2: AI 明確發出 EXIT_SIGNAL = true
    status := ra.ParseStructuredOutput()
    if status == nil || !status.ExitSignal {
        return false
    }
    
    // 雙重條件都滿足才退出
    return true
}
```

**為什麼需要雙重條件?**

| 場景 | 完成指標 | EXIT_SIGNAL | 結果 | 風險 |
|------|----------|------------|------|------|
| AI 還在思考 | < 2 | false | 繼續 ✅ | 無 |
| AI 迷茫 | 3+ | false | 繼續 ✅ | 無 |
| 誤判完成 | 3+ | false | 繼續 ✅ | 無 |
| **過早退出** | 3+ | false | ~~退出~~ ✅ | 被防止 |
| **無限迴圈** | < 2 | true | ~~退出~~ ✅ | 被防止 |
| 正確完成 | 3+ | true | 退出 ✅ | 無 |

### 3. 完成信號的層次

```
層次 1: 結構化信號（最可靠）
├─ EXIT_SIGNAL = true (100 分)
└─ 來自 ---COPILOT_STATUS--- 區塊

層次 2: 自然語言關鍵字（次可靠）
├─ 完成 / 完全完成 (10 分)
├─ 沒有更多工作 (15 分)
└─ 準備就緒 (10 分)

層次 3: 上下文線索（輔助）
└─ 輸出短小 (10 分) → 表示進度甚微

決策: 必須層次 1 + (層次 2 或層次 3)
```

## 模組互動流程

### 典型的迴圈執行流程

```
1. 取得使用者請求
   ↓
2. CLIExecutor.SuggestShellCommand()
   ├─ 執行 github-copilot-cli
   └─ 回傳 ExecutionResult
   ↓
3. OutputParser
   ├─ 提取程式碼區塊
   ├─ 提取選項列表
   └─ 清除 Markdown 格式
   ↓
4. ResponseAnalyzer
   ├─ 解析結構化輸出
   ├─ 計算完成分數
   ├─ 偵測測試迴圈
   ├─ 偵測卡住狀態
   └─ 檢查雙重條件 ← 關鍵決策點
   ↓
5. CircuitBreaker
   ├─ 記錄進度或錯誤
   ├─ 檢查熔斷狀態
   └─ 決定是否繼續
   ↓
6. 判斷:
   ├─ IsCompleted() = true → 😊 退出迴圈
   ├─ CircuitBreaker.IsOpen() = true → ⚠️ 開啟熔斷
   └─ 其他 → ↻ 進入下一迴圈
```

## 故障恢復策略

### 熔斷器狀態轉換

```
                    ┌─────────────┐
                    │   CLOSED    │
                    │ (正常運作)   │
                    └──────┬──────┘
                           │
                    失敗×3 │ (無進展/相同錯誤)
                           ↓
                    ┌─────────────┐
                    │    OPEN     │
                    │ (停止執行)   │
                    └──────┬──────┘
                           │
                    成功 × 1│
                           ↓
                    ┌─────────────┐
        失敗 ←─────┤ HALF_OPEN   │
                    │ (試探恢復)   │
                    └──────┬──────┘
                           │
                    成功 × 1│
                           ↓
                    ┌─────────────┐
                    │   CLOSED    │
                    │ (恢復正常)   │
                    └─────────────┘
```

## 數據流範例

### 正常完成流程

```json
// 第 1 輪
{
  "response": "---COPILOT_STATUS---\nSTATUS: CONTINUE\nEXIT_SIGNAL: false\n---END_STATUS---\n\n完成了第一步",
  "completion_score": 10,
  "completion_indicators": ["short_output"],
  "is_completed": false,
  "circuit_breaker_state": "CLOSED"
}

// 第 2 輪
{
  "response": "所有任務已完成。---COPILOT_STATUS---\nSTATUS: DONE\nEXIT_SIGNAL: true\n---END_STATUS---",
  "completion_score": 125,
  "completion_indicators": ["explicit_exit_signal", "完成"],
  "is_completed": true,  // 雙重條件滿足 ✅
  "circuit_breaker_state": "CLOSED"
}

結果: EXIT ✅
```

### 故障恢復流程

```json
// 第 1 輪: 錯誤
{
  "error": "Connection timeout",
  "circuit_breaker": {
    "state": "CLOSED",
    "same_error_loops": 1
  }
}

// 第 2-4 輪: 重複錯誤
{
  "error": "Connection timeout",
  "circuit_breaker": {
    "state": "CLOSED",
    "same_error_loops": 5  // 達到閾值
  }
}

// 第 5 輪
{
  "circuit_breaker": {
    "state": "OPEN"  // 🔴 熔斷器打開
  }
}

結果: STOP (等待手動介入)
```

## 性能優化

### 1. 完成檢測的速度

```
方案 A: 每迴圈都做 NLP 處理
├─ 成本: 高
├─ 準確率: 75%
└─ 風險: 容易過早退出

方案 B (採用): 結構化 + NLP 混合
├─ 結構化 (100ms, 99% 準確)
├─ NLP 僅作輔助 (10ms, 80% 準確)
├─ 成本: 低
└─ 準確率: 95%+
```

### 2. 記憶體使用

```
熔斷器狀態持久化:
├─ 最後 3 個錯誤: ~300 字節
├─ 狀態資訊: ~200 字節
├─ 總計: ~500 字節
└─ 遠小於內存限制
```

## 擴展點

### 1. 新增完成指標

在 `response_analyzer.go` 的 `CalculateCompletionScore()`:

```go
// 新增指標
newKeywords := []string{"新指標1", "新指標2"}
for _, keyword := range newKeywords {
    if strings.Contains(response, keyword) {
        score += 20
        completionIndicators = append(...)
        break
    }
}
```

### 2. 新增熔斷條件

在 `circuit_breaker.go` 新增方法:

```go
func (cb *CircuitBreaker) RecordCustomCondition(condition bool) {
    if condition {
        cb.openCircuit("自訂條件觸發")
    }
}
```

### 3. 新增狀態信號

在 `response_analyzer.go` 的 `CopilotStatus` 結構:

```go
type CopilotStatus struct {
    Status        string
    ExitSignal    bool
    TasksDone     string
    NewField      string  // 新增欄位
}
```

---

**設計文件版本**: 1.0  
**上次更新**: 2026-01-20  
**相關文件**: [IMPLEMENTATION_PROGRESS.md](./IMPLEMENTATION_PROGRESS.md)
